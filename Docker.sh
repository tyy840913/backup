...
get_latest_versions() {
    ...
    case $OS in
        "ubuntu"|"debian")
            LATEST_DOCKER_VERSION=$(apt-cache madison docker-ce | awk '{print $3}' | head -n 1 | cut -d':' -f2)
        ;;
        "centos"|"rhel"|"fedora")
            LATEST_DOCKER_VERSION=$(yum --showduplicates list docker-ce | grep 'docker-ce' | awk '{print $2}' | tail -n 1 | cut -d':' -f2)
        ;;
    esac
...
check_docker() {
    ...
    if [[ -n "$LATEST_DOCKER_VERSION" && "$installed_version" != "$LATEST_DOCKER_VERSION" ]]; then
        echo -e "${YELLOW}发现新版本! 最新可用版本为: $LATEST_DOCKER_VERSION${NC}"
        return 2
    fi
...
configure_mirror() {
    ...
    if [ -f "$DAEMON_JSON" ]; then
        ...
        if [[ ! "$replace_choice" =~ ^[Yy]$ ]]; then
            echo -e "${BLUE}已跳过 Docker 镜像加速器配置。${NC}"
            return
        fi
    fi
    ...
    if systemctl restart docker; then
        ...
        echo -e "${RED}Docker 服务重启失败，请手动检查: systemctl status docker${NC}"
        return 1
    fi
}
...
main() {
    ...
    case $docker_status in
        ...
        ;;
        1) read -p "未检测到 Docker，是否立即安装？(Y/n): " c
           if [[ "$c" =~ ^[Yy]?$ ]]; then
               install_or_update_docker
           else
               echo -e "${BLUE}已跳过 Docker 安装。${NC}"
           fi
        ;;
        2) read -p "检测到 Docker 新版本，是否立即更新？(Y/n): " c
           if [[ "$c" =~ ^[Yy]?$ ]]; then
               install_or_update_docker
           else
               echo -e "${BLUE}已跳过 Docker 更新。${NC}"
           fi
        ;;
    esac
    ...
    case $compose_status in
        ...
        ;;
        1) read -p "未检测到 Docker Compose，是否立即安装？(Y/n): " c
           if [[ "$c" =~ ^[Yy]?$ ]]; then
               install_or_update_compose
           else
               echo -e "${BLUE}已跳过 Compose 安装。${NC}"
           fi
        ;;
        2) read -p "检测到 Docker Compose 新版本，是否立即更新？(Y/n): " c
           if [[ "$c" =~ ^[Yy]?$ ]]; then
               install_or_update_compose
           else
               echo -e "${BLUE}已跳过 Compose 更新。${NC}"
           fi
        ;;
    esac
    ...
    if command -v docker &>/dev/null; then
        ...
        if systemctl is-active --quiet docker; then
            ...
            if docker info 2>/dev/null | grep -q "Registry Mirrors"; then
                echo -e "镜像加速: ${GREEN}已配置${NC}"
            else
                echo -e "镜像加速: ${YELLOW}未在 docker info 中检测到${NC}"
            fi
            
            if docker info 2>/dev/null | grep -q -E "HTTP Proxy:|HTTPS Proxy:"; then
                echo -e "Docker 代理: ${GREEN}已配置${NC}"
            else
                echo -e "Docker 代理: ${YELLOW}未在 docker info 中检测到${NC}"
            fi
        else
