...
read_with_esc_cancel() {
    ...
    # 设置一个陷阱，无论函数如何退出（正常、Ctrl+C等），都恢复终端
    trap 'stty sane' EXIT
    
    echo -ne "$prompt"
    stty -echo # 关闭回显以处理特殊字符
    ...
    trap - EXIT; stty sane # 确保正常退出时也恢复终端
}
...
enable_firewall() {
    ...
    if ufw status | grep -q "Status: active"; then
        ...
        if [[ $confirm =~ ^[Yy]$ ]]; then ufw enable; echo -e "${GREEN}✅ 防火墙已成功启用。${NC}"; else echo -e "${RED}❌ 操作已取消。${NC}"; fi
    fi
}
...
disable_firewall() {
    ...
    if ufw status | grep -q "Status: inactive"; then
        ...
        if [[ $confirm =~ ^[Yy]$ ]]; then ufw disable; echo -e "${GREEN}✅ 防火墙已成功关闭。${NC}"; else echo -e "${RED}❌ 操作已取消。${NC}"; fi
    fi
}
...
reset_firewall() {
    ...
    if [[ $confirm =~ ^[Yy]$ ]]; then ufw reset; echo -e "${GREEN}✅ 防火墙已重置。${NC}"; else echo -e "${RED}❌ 操作已取消。${NC}"; fi
}
    ...
    while true; do
        ...
        case $opt in
            1)
                ...
                if [ -z "$port" ]; then ufw allow from "$ip"; else if [[ "$proto" =~ ^(tcp|both)$ ]]; then ufw allow proto tcp from "$ip" to any port "$port"; fi; if [[ "$proto" =~ ^(udp|both)$ ]]; then ufw allow proto udp from "$ip" to any port "$port"; fi; fi
            ...
            2)
                ...
                read -p "协议 [tcp|udp|both] (默认 both): " proto; proto=${proto:-both}
                for p in "${ports_array[@]}"; do if [[ "$proto" =~ ^(tcp|both)$ ]]; then ufw allow "$p"/tcp; fi; if [[ "$proto" =~ ^(udp|both)$ ]]; then ufw allow "$p"/udp; fi; echo -e "${GREEN}✅ 端口 [$p] 已开放。${NC}"; done
            ...
            3)
                ...
                if [[ "$block_type" == "ip" ]]; then
                    ...
                    local ports_array=($USER_INPUT); read -p "协议 [tcp|udp|both] (默认 both): " proto; proto=${proto:-both}
                    for p in "${ports_array[@]}"; do if [[ "$proto" =~ ^(tcp|both)$ ]]; then ufw deny "$p"/tcp; fi; if [[ "$proto" =~ ^(udp|both)$ ]]; then ufw deny "$p"/udp; fi; echo -e "${GREEN}✅ 端口 [$p] 已封禁。${NC}"; done
                else echo -e "${RED}❌ 无效的选择。${NC}"; fi
                if ! pause; then return; fi
                ;;
            4)
                read_with_esc_cancel "请输入要删除的规则【编号】: "; local ret=$?; if [ $ret -ne 0 ]; then echo -e "\n${RED}操作取消。${NC}"; sleep 0.5; continue; fi
                read -p "您确定要删除规则【#$USER_INPUT】吗? (y/n): " confirm
                if [[ $confirm =~ ^[Yy]$ ]]; then ufw --force delete "$USER_INPUT"; echo -e "${GREEN}✅ 规则 #${USER_INPUT} 已删除。${NC}"; else echo -e "${RED}❌ 操作已取消。${NC}"; fi
                if ! pause; then return; fi
                ;;
...
manage_logs_menu() {
    while true; do
        ...
        echo -e "  1) 设置日志级别"; echo -e "  2) 查看最近日志"; echo -e "  3) 实时监控日志"; echo -e "\n  ${BLUE}0) 返回主菜单 (或在等待时按 ESC)${NC}"
        read -p "请选择 [0-3]: " opt
        case $opt in
            1)
                read -p "请输入日志级别 [低|中|高|完整|关闭]: " level_zh; local level_en=""
                case "$level_zh" in "低") level_en="low" ;; "中") level_en="medium" ;; "高") level_en="high" ;; "完整") level_en="full" ;; "关闭") level_en="off" ;; *) echo -e "${RED}❌ 无效的级别。${NC}";; esac
                if [ -n "$level_en" ]; then ufw logging "$level_en"; echo -e "${GREEN}✅ 日志级别已成功设置为: $level_zh${NC}"; fi
                if ! pause; then return; fi
                ;;
            2)
                echo -e "\n${YELLOW}--- 最近 50行 UFW 日志 ---${NC}"; if [ -f "/var/log/ufw.log" ]; then tail -n 50 /var/log/ufw.log; else echo -e "${RED}日志文件不存在。${NC}"; fi
                if ! pause; then return; fi
                ;;
            3)
                echo -e "\n${YELLOW}--- 实时监控 (按 Ctrl+C 退出) ---${NC}"; if [ -f "/var/log/ufw.log" ]; then tail -f /var/log/ufw.log; else echo -e "${RED}日志文件不存在。${NC}"; if ! pause; then return; fi; fi
                ;;
            0) return ;; *) echo -e "${RED}无效输入。${NC}"; if ! pause; then return; fi ;;
        esac
    done
}
...
manage_backup_menu() {
    while true; do
        clear
        echo -e "${YELLOW}--- 备份与恢复 ---${NC}"; echo -e "  1) 导出规则"; echo -e "  2) 导入规则"; echo -e "\n  ${BLUE}0) 返回主菜单 (或在等待时按 ESC)${NC}"
        read -p "请选择 [0-2]: " opt
        case $opt in
            1)
                local f="/root/ufw-backup-$(date +%Y%m%d).tar.gz"; read -p "输入备份路径 (默认: $f): " p; p=${p:-$f}
            ...
            2)
                ...
                if [ -f "$f" ]; then
                    ...
                    if [[ $c =~ ^[Yy]$ ]]; then
                        if tar -xzf "$f" -C /; then echo -e "${GREEN}✅ 配置已导入。${NC}"; read -p "立即重载防火墙? (y/n): " r; if [[ $r =~ ^[Yy]$ ]]; then ufw reload; echo -e "${GREEN}✅ 防火墙已重载。${NC}"; fi
...
main_menu() {
    while true; do
        clear
        echo -e "${GREEN}╔════════════════════════════════════════════════════════════╗${NC}"; echo -e "${GREEN}║${NC}              🛡️  ${YELLOW}UFW 防火墙管理器 v2025.07.11${NC}              ${GREEN}║${NC}"; echo -e "${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
        show_simple_status
        if [ -n "$STARTUP_MSG" ]; then echo -e "$STARTUP_MSG"; STARTUP_MSG=""; fi
        echo -e "\n${YELLOW}--- 基本操作 ---${NC}"; echo -e "  1) 启用防火墙"; echo -e "  2) 关闭防火墙"; echo -e "  3) 查看详细状态"; echo -e "  4) 重置防火墙"
        echo -e "\n${YELLOW}--- 高级功能 ---${NC}"; echo -e "  5) 管理防火墙规则"; echo -e "  6) 日志管理"; echo -e "  7) 备份与恢复"
        echo -e "\n${YELLOW}--------------------------------------------------------------${NC}"; echo -e "  ${BLUE}0) 退出脚本 (或在等待时按 ESC)${NC}"
        read -p "请输入您的选择 [0-7]: " choice
        case $choice in
            1) enable_firewall; if ! pause; then echo -e "\n${GREEN}按 ESC 退出... 再见！${NC}"; exit 0; fi ;;
            2) disable_firewall; if ! pause; then echo -e "\n${GREEN}按 ESC 退出... 再见！${NC}"; exit 0; fi ;;
            3) show_detailed_status; if ! pause; then echo -e "\n${GREEN}按 ESC 退出... 再见！${NC}"; exit 0; fi ;;
            4) reset_firewall; if ! pause; then echo -e "\n${GREEN}按 ESC 退出... 再见！${NC}"; exit 0; fi ;;
            5) custom_rule_manager ;;
            6) manage_logs_menu ;;
            7) manage_backup_menu ;;
            0) echo -e "\n${GREEN}感谢使用，再见！${NC}"; exit 0 ;;
            *) echo -e "${RED}无效的输入。${NC}"; if ! pause; then echo -e "\n${GREEN}按 ESC 退出... 再见！${NC}"; exit 0; fi ;;
        esac
    done
}
