...
show_mirror_menu() {
  ...
  for name in "${!MIRRORS[@]}"; do
      ...
      # 根据配置类型过滤源
      case $config_type in
          1) # Debian System Sources: 只显示非PVE官方的通用源
              if [[ "$name" != "Proxmox"* ]]; then
                  available_mirrors+=("$name")
              fi
              ;;
          2|3) # PVE or LXC Sources: 显示PVE官方源和提供PVE镜像的国内源
              if [[ "$name" == "Proxmox"* ]] || \
                 [[ "$name" == "清华大学" ]] || \
                 [[ "$name" == "阿里云" ]] || \
                 [[ "$name" == "中科大" ]] || \
                 [[ "$name" == "龙芯(LoongArch)" ]]; then
                  available_mirrors+=("$name")
              fi
              ;;
      esac
...
disable_subscription_nag() {
    ...
    # 确定目标文件
    # PVE 8.1+ 的弹窗逻辑在 pvemanagerlib.js
    if [[ -f "$pvemanager_js" ]] && grep -q "PVE.UI.Debian.activateSubscription" "$pvemanager_js"; then
        js_file=$pvemanager_js
        echo -e "\n${YELLOW}检测到 PVE 8.1+ 环境，目标文件: pvemanagerlib.js${NC}"
    elif [ ! -f "$js_file" ]; then
        echo -e "${RED}错误: 未找到JS目标文件。可能是 proxmox-widget-toolkit 包未安装。${NC}"
        sleep 2
        return
    else
        echo -e "\n${YELLOW}检测到 PVE 7.x/8.0.x 环境，目标文件: proxmoxlib.js${NC}"
    fi
...
apply_debian_mirror() {
  ...
  # 针对龙芯源和通用Debian源的特殊路径处理
  local debian_path_prefix="debian"
  if [[ "$ARCH" == "loongarch64" ]] && [[ "$base_url" == "http://mirrors.loongnix.cn" ]]; then
      debian_path_prefix="debian" # 龙芯源的debian目录就是debian
  fi
  cat > "$config_file" <<EOF
# Debian ${DEBIAN_VER} - Generated by PVE Tool
deb ${base_url}/${debian_path_prefix}/ ${DEBIAN_VER} ${components}
deb ${base_url}/${debian_path_prefix}/ ${DEBIAN_VER}-updates ${components}
deb ${base_url}/${debian_path_prefix}-security/ ${DEBIAN_VER}-security ${components}
EOF
}
apply_pve_mirror() {
