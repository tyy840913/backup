...
command -v curl &>/dev/null && COMMAND="curl -so" || command -v wget &>/dev/null && COMMAND="wget -qO" || { 
    echo -e "\e[1;31m[错误] 未找到curl或wget，请安装其中一个工具\e[0m" >&2
    exit 1
}
...
check_port() {
    ...
    if [[ $udp_ports -lt 1 ]]; then
        ...
        echo -e "\e[1;32m[成功] 端口调整完成。如果安装后连接失败，请访问/restart重启\e[0m"
...
echo -e "\e[1;32m[信息] 已选择IP地址: $HOST_IP。如果安装后连接失败，请尝试重新安装。\e[0m"
        ...
        if ! pgrep -x "$(basename "$HY2_BINARY")" > /dev/null; then
            ...
            for i in {1..3}; do
                if start_service; then
                    ...
                    # 如果3次都失败，退出脚本
                    echo -e "\e[1;31m[错误] 重启Hysteria2服务失败，已达到最大重试次数\e[0m"
                    exit 1
                fi
                sleep 5  # 等待5秒再重试
            done
...
# 在后台启动监控进程
echo -e "\e[1;33m[信息] 正在启动服务监控进程...\e[0m"
monitor_service &
...
echo -e "\e[1;33m[信息] 脚本来源: https://github.com/eooce/sing-box\e[0m"
echo -e "\e[1;32m[成功] 所有操作已完成!\e[0m\n"

# 保持脚本运行
echo -e "\e[1;33m[信息] 监控进程已在后台运行，将每分钟检查一次服务状态\e[0m"
while true; do 
