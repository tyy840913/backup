...
curl -u root:421121 https://backup.woskee.dpdns.org/xiaoya | tar -xf - -C /etc
...
if [ $? -eq 0 ]; then
    ...
    # 定义定时任务
    # 这个定时任务会在每3天的午夜0点0分执行备份
    CRON_JOB="0 0 */3 * * tar -cf - -C /etc --exclude=xiaoya/data xiaoya | curl -T - https://backup.woskee.dpdns.org/update/xiaoya >/dev/null 2>&1"
